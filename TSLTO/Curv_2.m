function X_pre = Curv_2(G,U1,X_0,U3,L,beta,alpha,Y,T,V,tau1,tau2,delta,ita,epsilon,maxiter_1,maxiter_2,ro)
%X_0: 初始点 即U1
%tau1: min tau
%tau2: max tau
...
X_pre = X_0;
q = 1;
c = f_U2(G,U1,X_pre,U3,L,beta,alpha,Y,T,V);
tau = tau1;
F_pre = c;
for i = 1:maxiter_2
    %X_pre = X_cur;
    [g_pre,a] = Gradient_for_U_2(G,U1,X_pre,U3,L,beta,alpha,Y,T,V,3);
    ...
    for j = 1:maxiter_1
        ...
        F = f_U2(G,U1,X_cur,U3,L,beta,alpha,Y,T,V);
        if F < c - 1/2*ro*tau*norm(a,"fro")^2
            break;
        end
    end
    g_cur = Gradient_for_U_2(G,U1,X_cur,U3,L,beta,alpha,Y,T,V,1);
    q = ita*q + 1;
    c = (1/q).*(ita*q*c + F);
    s = X_cur - X_pre;
    ...
    %     fprintf('Curv_2: iterations = %d   value=%f\n', i, F_pre);
    % end
    F_pre = F;
end
%fprintf("fU2 final: %f\n  iter: %d\n",F_pre,i);